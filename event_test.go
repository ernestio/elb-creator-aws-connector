...
func testSetup() (chan *nats.Msg, chan *nats.Msg) {
	...
	nc.ChanSubscribe("elb.create.aws.done", doneChan)
	nc.ChanSubscribe("elb.create.aws.error", errChan)
	...
	Convey("Given I an event", t, func() {
		Convey("With valid fields", func() {
			...
			Convey("When completing the event", func() {
				...
				e.Process(valid)
				e.Complete()
				Convey("It should produce a elb.create.aws.done event", func() {
			...
			Convey("When erroring the event", func() {
				...
				Convey("It should produce a elb.create.aws.error event", func() {
					msg, timeout := waitMsg(errored)
					So(msg, ShouldNotBeNil)
